 ...
 * can be individually earlier by calling vuObserverDeactivate().
 *
 ...
 * vuEngineCreate<ObserverType>Observer()) is set to VU_<OBSERVER_TYPE>_CREATION_ERROR_INTERNAL
 * for all observer types where applicable. Only when creating the device pose observer it will
 * be set to the error code VU_DEVICE_POSE_CREATION_ERROR_TRACKING_UNAVAILABLE.
 *
